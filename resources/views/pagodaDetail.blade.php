        ...
        <div class="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="h-64 sm:h-96 w-full bg-gray-100 relative overflow-hidden">
                    @if($pagoda->photo)
                        <img src="{{ asset($pagoda->photo) }}" alt="{{ $pagoda->name }}" class="w-full h-full object-cover">
                    @else
                        <img src="https://placehold.co/1200x600/f3f4f6/0891b2?text={{ urlencode($pagoda->name) }}" alt="{{ $pagoda->name }}" class="w-full h-full object-cover">
                    @endif
                    
                    @if($pagoda->status === 'famous')
                        <div class="absolute top-4 right-4 bg-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md">
                            ⭐ တန်ခိုးကြီးဘုရား
                        </div>
                    @endif
                </div>
                ...
                <div class="p-6 sm:p-10">
                    <div class="border-b border-gray-100 pb-6 mb-6">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2 text-xs font-semibold text-gray-500">
                            ...
                            @if($pagoda->township)
                                <span class="bg-gray-100 px-2.5 py-1 rounded-md">{{ $pagoda->township->name }} မြို့နယ်</span>
                            @endif

                            @if($pagoda->township && $pagoda->township->district)
                                <span class="text-gray-300">/</span>
                                <span class="bg-gray-100 px-2.5 py-1 rounded-md">{{ $pagoda->township->district->name }} ခရိုင်</span>
                            @endif

                            @if($pagoda->township && $pagoda->township->district && $pagoda->township->district->division)
                                <span class="text-gray-300">/</span>
                                <a href="{{ route('division.show', $pagoda->township->district->division->id) }}" class="bg-cyan-50 text-cyan-600 hover:bg-cyan-100 px-2.5 py-1 rounded-md transition decoration-none">
